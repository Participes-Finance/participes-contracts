...
describe("MasterChef", function () {
  ...
  context("With ERC/LP token added to the field", function () {
    ...
    it("should give out PARTSs only after farming time", async function () {
      // 100 per block farming rate starting at block 100 with bonus until block 1000
      ...
      let blockSkipAmount = await (startBlockNum - 13 - (await ethers.provider.getBlockNumber()))
    ...
    it("should distribute PARTSs properly for each staker", async function () {
      ...
      expect(await this.parts.totalSupply()).to.equal("55000")
      expect(await this.parts.balanceOf(this.dev.address)).to.equal("5000")
      // Alice should have: 5666 + 10*2/7*1000 + 10*2/6.5*1000 = 11600
      expect(await this.parts.balanceOf(this.alice.address)).to.equal("11600")
      // Bob should have: 6190 + 10*1.5/6.5 * 1000 + 10*1.5/4.5*1000 = 11831
      expect(await this.parts.balanceOf(this.bob.address)).to.equal("11831")
      // Carol should have: 2*3/6*1000 + 10*3/7*1000 + 10*3/6.5*1000 + 10*3/4.5*1000 + 10*1000 = 26568
      expect(await this.parts.balanceOf(this.carol.address)).to.equal("26568")
    ...
    it("should give proper PARTSs allocation to each pool", async function () {
      ...
      this.chef = await this.MasterChef.deploy(this.parts.address, this.dev.address, this.dev.address, "100", "400")
      await this.parts.transferOwnership(this.chef.address)
      ...
      await this.chef.add("10", this.lp2.address, 0, true)
      // Alice should have 10*1000 pending reward
      expect(await this.chef.pendingParts(0, this.alice.address)).to.equal("10000")
      // Bob deposits 10 LP2s at block 425
      await advanceBlockTo("424")
      await this.chef.connect(this.bob).deposit(1, "5", { from: this.bob.address })
      // Alice should have 10000 + 5*1/3*1000 = 11666 pending reward
      expect(await this.chef.pendingParts(0, this.alice.address)).to.equal("11666")
      await advanceBlockTo("430")
      // At block 430. Bob should get 5*2/3*1000 = 3333. Alice should get ~1666 more.
      expect(await this.chef.pendingParts(0, this.alice.address)).to.equal("13333")
      expect(await this.chef.pendingParts(1, this.bob.address)).to.equal("3333")
    })
