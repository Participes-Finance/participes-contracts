...
masterchef: public(address) # masterchef contract
...
gauges: public(uint256[1000000000])

# we increment values by 1 prior to storing them here so we can rely on a value
# of zero as meaning the gauge has not been set
gauge_types_: HashMap[uint256, int128]

vote_user_slopes: public(HashMap[address, HashMap[uint256, VotedSlope]])  # user -> pid -> VotedSlope
vote_user_power: public(HashMap[address, uint256])  # Total vote power used by user
last_user_vote: public(HashMap[address, HashMap[uint256, uint256]])  # Last user vote's timestamp for each gauge address
...
points_weight: public(HashMap[uint256, HashMap[uint256, Point]])  # pid -> time -> Point
changes_weight: HashMap[uint256, HashMap[uint256, uint256]]  # pid -> time -> slope
time_weight: public(HashMap[uint256, uint256])  # pid -> last scheduled time (next week)
...
@external 
def set_masterchef(_masterchef: address):
    """
    @notice Set MasterChef contract address 
    @param _masterchef MasterChef contract address
    """
    assert msg.sender == self.admin
    self.masterchef = _masterchef
...
@external
@view
def gauge_types(_pid: uint256) -> int128:
    """
    @notice Get gauge type for address
    @param _pid Gauge address
    @return Gauge type id
    """
    gauge_type: int128 = self.gauge_types_[_pid]
    assert gauge_type != 0
...
@internal
def _get_weight(pid: uint256) -> uint256:
    """
    @notice Fill historic gauge weights week-over-week for missed checkins
            and return the total for the future week
    @param pid Address of the gauge
    @return Gauge weight
    """
    t: uint256 = self.time_weight[pid]
    if t > 0:
        pt: Point = self.points_weight[pid][t]
        for i in range(500):
            if t > block.timestamp:
                break
            t += WEEK
            ...
            if pt.bias > d_bias:
                pt.bias -= d_bias
                d_slope: uint256 = self.changes_weight[pid][t]
                pt.slope -= d_slope
            else:
                pt.bias = 0
                pt.slope = 0
            self.points_weight[pid][t] = pt
            if t > block.timestamp:
                self.time_weight[pid] = t
        return pt.bias
    else:
        return 0
...
@external
def add_gauge(pid: uint256, gauge_type: int128, weight: uint256 = 0):
    """
    @notice Add gauge `addr` of type `gauge_type` with weight `weight`
    @param pid Gauge address
    @param gauge_type Gauge type
    @param weight Gauge weight
    """
    # assert msg.sender == self.admin 
    assert msg.sender == self.masterchef
    assert (gauge_type >= 0) and (gauge_type < self.n_gauge_types)
    assert self.gauge_types_[pid] == 0  # dev: cannot add the same gauge twice

    n: int128 = self.n_gauges
    self.n_gauges = n + 1
    self.gauges[n] = pid

    self.gauge_types_[pid] = gauge_type + 1
    ...
    if weight > 0:
        ...
        self.points_weight[pid][next_time].bias = weight

    if self.time_sum[gauge_type] == 0:
        self.time_sum[gauge_type] = next_time
    self.time_weight[pid] = next_time

    log NewGauge(pid, gauge_type, weight)
...
@external
def checkpoint_gauge(pid: uint256):
    """
    @notice Checkpoint to fill data for both a specific gauge and common for all gauges
    @param pid Gauge pid
    """
    self._get_weight(pid)
    self._get_total()


@internal
@view
def _gauge_relative_weight(pid: uint256, time: uint256) -> uint256:
    """
    ...
    if _total_weight > 0:
        gauge_type: int128 = self.gauge_types_[pid] - 1
        _type_weight: uint256 = self.points_type_weight[gauge_type][t]
        _gauge_weight: uint256 = self.points_weight[pid][t].bias
...
@external
@view
def gauge_relative_weight(pid: uint256, time: uint256 = block.timestamp) -> uint256:
    """
    @notice Get Gauge relative weight (not more than 1.0) normalized to 1e18
            (e.g. 1.0 == 1e18). Inflation which will be received by it is
            inflation_rate * relative_weight / 1e18
    ...
    @return Value of relative weight normalized to 1e18
    """
    return self._gauge_relative_weight(pid, time)


@external
def gauge_relative_weight_write(pid: uint256, time: uint256 = block.timestamp) -> uint256:
    """
...
@internal
def _change_gauge_weight(pid: uint256, weight: uint256):
    # Change gauge weight
    # Only needed when testing in reality
    gauge_type: int128 = self.gauge_types_[pid] - 1
    old_gauge_weight: uint256 = self._get_weight(pid)
    ...
    log NewGaugeWeight(pid, block.timestamp, weight, _total_weight)


@external
def change_gauge_weight(pid: uint256, weight: uint256):
    """
    @notice Change weight of gauge `pid` to `weight`
    @param pid `GaugeController` contract address [TODO]: this is misleading, the "addr" is actually for gauge not for GaugeController?
    @param weight New Gauge weight
    """
    assert msg.sender == self.admin
    self._change_gauge_weight(pid, weight)


@external
def vote_for_gauge_weights(pid: uint256, _user_weight: uint256):
    """
    @notice Allocate voting power for changing pool weights
    @param pid Gauge which `msg.sender` votes for
    ...
    n_gauges: int128 = self.n_gauges
    next_time: uint256 = (block.timestamp + WEEK) / WEEK * WEEK
    assert lock_end > next_time, "Your token lock expires too soon"
    assert (_user_weight >= 0) and (_user_weight <= 10000), "You used all your voting power"
    assert block.timestamp >= self.last_user_vote[msg.sender][pid] + WEIGHT_VOTE_DELAY, "Cannot vote so often"

    gauge_type: int128 = self.gauge_types_[pid] - 1
    assert gauge_type >= 0, "Gauge not added"
    # Prepare slopes and biases in memory
    old_slope: VotedSlope = self.vote_user_slopes[msg.sender][pid]
    old_dt: uint256 = 0
    ...
    old_weight_bias: uint256 = self._get_weight(pid)
    old_weight_slope: uint256 = self.points_weight[pid][next_time].slope
    old_sum_bias: uint256 = self._get_sum(gauge_type)
    old_sum_slope: uint256 = self.points_sum[gauge_type][next_time].slope

    self.points_weight[pid][next_time].bias = max(old_weight_bias + new_bias, old_bias) - old_bias
    self.points_sum[gauge_type][next_time].bias = max(old_sum_bias + new_bias, old_bias) - old_bias
    if old_slope.end > next_time:
        self.points_weight[pid][next_time].slope = max(old_weight_slope + new_slope.slope, old_slope.slope) - old_slope.slope
        self.points_sum[gauge_type][next_time].slope = max(old_sum_slope + new_slope.slope, old_slope.slope) - old_slope.slope
    else:
        self.points_weight[pid][next_time].slope += new_slope.slope
        self.points_sum[gauge_type][next_time].slope += new_slope.slope
    if old_slope.end > block.timestamp:
        # Cancel old slope changes if they still didn't happen
        self.changes_weight[pid][old_slope.end] -= old_slope.slope
        self.changes_sum[gauge_type][old_slope.end] -= old_slope.slope
    # Add slope changes for new slopes
    self.changes_weight[pid][new_slope.end] += new_slope.slope
    self.changes_sum[gauge_type][new_slope.end] += new_slope.slope

    self._get_total()

    self.vote_user_slopes[msg.sender][pid] = new_slope

    # Record last action time
    self.last_user_vote[msg.sender][pid] = block.timestamp

    log VoteForGauge(block.timestamp, msg.sender, pid, _user_weight)


@external
@view
def get_gauge_weight(pid: uint256) -> uint256:
    """
    @notice Get current gauge weight
    @param pid Gauge address
    @return Gauge weight
    """
    return self.points_weight[pid][self.time_weight[pid]].bias
