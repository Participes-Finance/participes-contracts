...
N_COINS: constant(uint256) = 1 # [TODO]: Use dynamic array or input fixed amount of tokens for fee distribution
...
def __init__(
    ...
    @param _token Fee token addresses
...
def ve_for_at(_user: address, _timestamp: uint256) -> uint256:
    """
    @notice Get the xPARTS balance for `_user` at `_timestamp`
    @param _user Address to query balance for
    @param _timestamp Epoch time
    @return uint256 xPARTS balance
    """
...
def checkpoint_total_supply():
    """
    @notice Update the xPARTS total supply checkpoint
...
def _claim(addr: address, ve: address, _last_token_time: uint256) -> uint256[N_COINS]:
    ...
    to_distribute: uint256[N_COINS] = [0] # [TODO]: figure out how to handle coins (see other todo in file)
...
def claim(_addr: address = msg.sender) -> (uint256[N_COINS]):
    ...
    @dev Each call to claim look at a maximum of 50 user xPARTS points.
         For accounts with many xPARTS related actions, this function
...
def claim_many(_receivers: address[20]) -> bool:
    ...
    @dev Used to claim for many accounts at once, or to make
         ...
         has significant xPARTS history
...
def burn(_coin: address) -> bool:
    """
    @notice Receive PARTS into the contract and trigger a token checkpoint
    @param _coin Address of the coin being received (must be PARTS)
    @return bool success
    """
    assert not self.is_killed
...
def kill_me():
    ...
    @dev Killing transfers the entire PARTS balance to the emergency return address
